...
export function shouldCompress(contentType: string | null): boolean {
    ...
    // Compress text and common structured formats
 ...
 * Returns a function that takes compressed bytes and returns decompressed bytes.
 */
export function createDecompressor(encoding: string): ((data: Uint8Array) => Uint8Array) | null {
    const enc = encoding.toLowerCase().trim();
    if (enc === 'gzip') {
        return (data) => new Uint8Array(zlib.gunzip(data));
    }
    if (enc === 'deflate') {
        return (data) => {
            // Servers may send raw deflate (no zlib header) or zlib-wrapped deflate.
            // Try inflate first (zlib-wrapped), fall back to inflateRaw.
            try {
                return new Uint8Array(zlib.inflate(data));
            } catch {
                return new Uint8Array(zlib.inflateRaw(data));
            }
        };
    }
...
/**
 * Streaming decompressor state for incremental decompression.
 * Use when body arrives in multiple chunks (e.g., HTTP streaming).
 */
export class StreamingDecompressor {
    ...
    /** Decompress a chunk. Returns decompressed bytes, or original if no decompressor. */
 ...
 * Returns a function that takes raw bytes and returns compressed bytes.
 */
export function createCompressor(encoding: string, level = zlib.DEFAULT_COMPRESSION): ((data: Uint8Array) => Uint8Array) | null {
    const enc = encoding.toLowerCase().trim();
    if (enc === 'gzip') {
        return (data) => new Uint8Array(zlib.gzip(data, level));
    }
    if (enc === 'deflate') {
        return (data) => new Uint8Array(zlib.deflate(data, level));
    }
    return null;
}
